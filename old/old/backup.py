...
class SoftDipsSettings:
    def __init__(self, yaml_file):
        ...
        self.settings = self.load_yaml(yaml_file)
    ...
    def game_settings_found(self, game_code, region) -> bool:
        ...
        return bool(self.settings.get(game_code, {}).get(region, {}))

    def get_game_name(self, game_code, region):
        return self.settings.get(game_code, {}).get(region, {}).get('game_name', "Unknown Game")
    ...
    def print_settings(self, game_code, region):
        game_settings = self.settings.get(game_code, {}).get(region, {})
    ...
    def generate_image(self, path, game_code, region):
        game_settings = self.settings.get(game_code, {}).get(region, {})
        if not game_settings:
            print(f"No settings found for game {game_code} in {region} region.")
            return
        ...
        settings_count = sum(1 for _ in self.iter_settings(game_code, region))
        ...
        for setting in self.iter_settings(game_code, region):
    ...
    def iter_settings(self, game_code, region):
    ...
    def set_game_settings(self, game_code, region, new_settings):
        ...
        if game_code not in self.settings:
            self.settings[game_code] = {}

        # Set the new settings, replacing the old ones
        self.settings[game_code][region] = new_settings
        print(f"Settings for '{game_code}' in '{region}' have been updated.")

    def enrich_settings(self, game_id, path, language):
        """
        ...
        """
        game_settings = self._read_game_settings_from_path(path, language)
        self.set_game_settings(game_id, language, game_settings)
        self.save_yaml()
        # self.print_settings(game_id, language)

    def _read_game_settings_from_path(self, file_path: str, language: str):
        """
                ...
                for file_info in zip_ref.infolist():
                    if file_info.filename.endswith('.p1'):
                        with zip_ref.open(file_info.filename) as file:
                            ...
                            if index != -1:
                                dip_pointer = self.get_dip_pointer(data, language)
                                if dip_pointer < len(data):
                                    game_name = self.get_rom_game_name(data, dip_pointer)
                                    special_settings, description_index = self.get_special_settings(data, dip_pointer)
                                    simple_settings = self.get_simple_settings(data, dip_pointer, description_index)
                                    return {
                                        'game_name': game_name,
                                    ...
                                    raise ValueError("Pointer out of range while reading game settings.")
                            else:
                                raise ValueError("NEO GEO string not found in the file.")
                        
                        os.remove(temp_file_path)
                        os.rmdir(temp_dir)
                        break
                else:
                    raise ValueError(f"No .p1 file found in the zip archive ({file_path}).")
        else:
    ...
    def get_special_settings(self, data, dip_pointer):
        special_settings = []
    ...
    def get_simple_settings(self, data, dip_pointer, description_index):
        simple_settings = []
...
if __name__ == "__main__":
    dips = SoftDipsSettings("dips.yaml")
    if dips.game_settings_found(game_code="zedblade", region='EU') == False:
        dips.enrich_settings('zedblade', './rom/zedblade.zip', 'EU')
        dips.print_settings('zedblade', 'EU')
        dips.generate_image(game_code="zedblade", region="EU", path="./")